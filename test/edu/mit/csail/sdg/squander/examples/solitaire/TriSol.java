package edu.mit.csail.sdg.squander.examples.solitaire;

import edu.mit.csail.sdg.annotations.Ensures;
import edu.mit.csail.sdg.annotations.Let;
...
public class TriSol {
    ...
    @Let(var  = "dst",
         expr = "this.neighborCells[via].elems.(this.neighborCells[src][via]) @- src")
    @Ensures({
        "return = states",
        ...
        "all i: {1 ... (return.length - 1)} | " +
           "(exists src, via: {0 ... (return[i].cells.length - 1)} | " +
               "src != via && " + 
               "this.neighborCells[src][via] != 0 && " +
               "one ($dst) && " +
               ...
               "(all x: {0 ... (return[i].cells.length - 1)} | " +
